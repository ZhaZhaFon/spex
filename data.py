...
class LibriMixInformed(Dataset):
    ...
    def __len__(self):
        ...
        if self.debug:
            return len(self.data_aux_list) // 40
        else:
            return len(self.data_aux_list)
    ...
    def __getitem__(self, idx):
        ...
        #      (len,) (len,) (len, ), len=segment*sample_rate
        aux_len = self.seg_len_aux
        valid_len = self.seg_len_aux
        spk_idx = torch.tensor(int(utt_id.split('-')[0]))
        return mixture, source, enroll, aux_len, valid_len, spk_idx
